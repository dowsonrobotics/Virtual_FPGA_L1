...
* Number and characteristics of AXI4 memory interfaces, if any.
  Characteristics include AXI4 address and data bus width.

* Number and characteristics of AXI4 interfaces to other resources, if
  any (UART, GPIO, ...).  Characteristics include AXI4 address and
  data bus width.

These are specified as part of the application, declaratively, in a
JSON file (e.g., `VF_Specs.json`) at the top-level of the application
repo.
...
`VF_TestApp/VF_Specs.json` in the diagram).  This is a one-time `make`
for a given JSON spec file.
